...
<div class="bg-white p-6 rounded-xl shadow space-y-4">
    <h3 class="text-lg font-semibold mb-2">Edit Kelas: {{ $class->name }}</h3>
    ...
    <form action="{{ route('admin.classes.update', $class->id) }}" method="POST" class="space-y-4">
        ...
        <div>
            ...
            <select name="grade" class="w-full border rounded px-3 py-2 text-sm">
        ...
        <div>
            ...
            <select name="homeroom_teacher_id" class="w-full border rounded px-3 py-2 text-sm">
                ...
                @foreach($teachers as $t)
                    <option value="{{ $t->id }}" @selected(old('homeroom_teacher_id', $class->homeroom_teacher_id) == $t->id)>
        ...
        <div x-data="{ open: true }" class="border rounded-lg p-3">
            ...
            <div x-show="open" x-collapse class="mt-3 space-y-2">
                @php
                    $selectedTeachers = $class->teachers->pluck('id')->toArray();
                @endphp
                @foreach($teachers as $t)
                    <label class="flex items-center gap-2 text-sm">
                        <input type="checkbox" name="teacher_ids[]"
                               value="{{ $t->id }}"
                               @checked(in_array($t->id, old('teacher_ids', $selectedTeachers)))
